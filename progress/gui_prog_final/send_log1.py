...
oo = open('log1c.txt', 'r', os.O_NONBLOCK)
mm = open('log1c.txt', 'r', os.O_NONBLOCK)
...
def send(Ax, Ay, Az, Av, lon, lat, eps, waktu, petugas, rute):
    ...
    j = str(rute)
    waktus = str(waktu)
    print (" x=%s y=%s z=%s va=%s lat=%s lon=%s error=%s petugas=%s rute=%s waktu=%s"%(b,c,d,e,f,g,h,i,'11',waktus))
    ...
    try:
        cur.execute(
            "INSERT INTO data_indek (x, y, z, va, lat, lon, epx, id_petugas, id_kereta, waktu) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (b, c, d, e, f, g, h, i, '11', waktus))
        db.commit()
        print ("sukses")
    except:
        print ("Gagal")
